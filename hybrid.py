...
def hybrid_encrypt(plaintext: bytes, recipient_pub_pem: bytes, sender_priv_pem: bytes = None, sender_id: str = "") -> str:
    ...
    if sender_priv_pem:
        ...
        to_sign = (bundle["enc_session_key"] + bundle["iv"] + bundle["ciphertext"] + timestamp).encode('utf-8')
...
def hybrid_decrypt(bundle_json: str, recipient_priv_pem: bytes, sender_pub_pem: bytes = None):
    ...
    iv = ub64(bundle["iv"])
    ct = ub64(bundle["ciphertext"])
    tag = ub64(bundle["tag"])
    sig_b64 = bundle.get("signature")

    session_key = rsa_decrypt_private(recipient_priv, enc_session)

    if sig_b64 and sender_pub_pem is not None:
        to_sign = (bundle["enc_session_key"] + bundle["iv"] + bundle["ciphertext"] + bundle["timestamp"]).encode('utf-8')
        sender_pub = load_public_pem(sender_pub_pem)
        if not verify_rsa_signature(sender_pub, to_sign, ub64(sig_b64)):
            raise ValueError("Signature verification failed")

    plaintext = aes_gcm_decrypt(session_key, iv, ct, tag)
    return plaintext
